...
- 创建 `wpw/active`、`wpw/archived`、`wpw/knowledge/experiences` 目录
...
构建四层知识图谱（需求->模块->文件->代码元素），后续各阶段命令可调用 `wpw graph context` 获取精准代码上下文。国内网络环境建议在 `workflow.config.yaml` 配置 `graph.embedding.mirror: modelscope`。
