...
> 出问题时查「六、排坑经验」。最后维护：2026-08-16（第 1 章十一稿：模板目录重组为 1.1-1.5 五节含 14 个三级节、章名"绪论"->"引言"、国内外现状按机构属地划分、[1]-[42] 文献终版核实，已跑流程）。
...
## 八、当前状态快照（2026-08-16，第 1 章十一稿模板目录重组 + 文献终版核实后）

| 项 | 状态 |
|---|---|
| docx 封面/摘要/关键词/数据集表 | ✅ 已填（盲审留空个人信息） |
| 第 1 章 | ✅ 十一稿已导入（章名"引言"：1.1 研究背景（1.1.1-1.1.4）/ 1.2 研究意义（1.2.1-1.2.2）/ 1.3 国内外研究现状（1.3.1 国外五向编号段 + 1.3.2 国内五向编号段（按机构属地划分）+ 1.3.3 不足归纳）/ 1.4 研究目标与研究方法（1.4.1 目标含背景引入 / 1.4.2 方法含验证设计，无 RQ 编号）/ 1.5 创新点（1.5.1-1.5.4）；无章节安排节；表 1-1 保留） |
| 第 5 章 | ✅ 结构未变（表 5-1 仍用 RQ1-RQ3 表述，与第 1 章新口径的统一待处理） |
...
| 参考文献 | ✅ 42 条 GB/T 7714 + 正文上标（66 个上标 run；**[1]-[42] 已终版核实**，仅 [34] CatCoder TOSEM 卷期待刊出后补；作者前三位补全仍待 Word 端处理） |
| LaTeX 版 | ⚠️ 落后一轮（chap1.tex 仍为旧"绪论"结构，需跑 `build_latex.py` 重生成） |
