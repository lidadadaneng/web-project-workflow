...
> 出问题时查「六、排坑经验」。最后维护：2026-08-23（第 3 章六节重构成稿：新写 3.1 总体框架 / 3.2 流程约束架构 / 3.4 Spec 增量演化，3.3 增 RIG/KGCompass 对比表（表3-4），图表编号重排（图3-1~3-5、表3-1~3-5），第 5 章表号引用同步改表3-5，docx+LaTeX 已同步）。
...
| `import_chapters.py` | md 第 1/2/3/5 章 -> docx 对应章区重建（含样式克隆、列表/表格/代码/粗体转换） |
...
# 1. 重建章节（会重置第1/2/3/5章区域，公式回到LaTeX文本，引用回到普通文本）
...
三步**顺序不可换**。如果改了参考文献清单（unify_refs.py 的 REFS 列表，现为 [1]-[59]），先更新 `unify_refs.py` 顶部的 `REFS` 列表与 `ok_citation` 上限再跑第 3 步。
...
1. 更新目录域：Ctrl+A -> F9；
2. 补插图：正文提到的 图3-1~3-4、图5-1~5-4（5-1 总体架构 / 5-2 功能模块 / 5-3 方案 D 运行效果截图 / 5-4 压缩权衡曲线）等需插入图片文件（清单见各章 md 末尾「本章图表清单」）；
...
| 第 3 章 | ✅ **六节重构成稿已导入**（3.1 方法总体框架（闭环总图+三机制与三目标映射）/ 3.2 流程约束架构（六阶段+三段式契约+状态留痕+上下文请求与消费）/ 3.3 能力-代码双层知识模型（本体/多语言解析/多源证据/表3-4 RIG/KGCompass 对比）/ 3.4 Spec 增量知识演化机制（触发单元/冷启动/沉淀与增量更新）/ 3.5 图谱驱动的任务上下文生成（算法 3-1+式 3-1~3-4）/ 3.6 小结；图3-1/3-2 待绘制，旧占位"待导入"已清零；小节号引用违规已清除） |
| 第 5 章 | ✅ 结构未变（表5-1 仍用 RQ1-RQ3 表述，与第 1 章新口径的统一待处理；"第 3 章表3-4"引用已改"表3-5"） |
| 第 4/6 章 | ⬜ 占位待写（优先级：第 4 章（含新写 4.5 增量维护）> 第 6 章；素材在 index.填充版备份.md） |
| 公式 | ✅ OMML 原生（显示 7 处 + 行内 8 段，失败 0） |
| 参考文献 | ✅ 59 条 GB/T 7714 + 正文上标（[1]-[42] 已终版核实；[43]-[57][59] 第 2 章新增、题录未逐条核实；[58] ContextBench 为**占位条目待题录**；作者前三位补全待 Word 端） |
| LaTeX 版 | ✅ 已同步（chap1-3/5 成稿 + chap4/6 骨架；myrefs.bib 尚未建，引用为明文 [n]） |
