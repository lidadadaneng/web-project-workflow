...
> 出问题时查「六、排坑经验」。最后维护：2026-08-23（第 4 章成稿及图4-1~图4-5已同步至 DOCX 与 LaTeX；第 3 章六节重构稿保持同步）。
...
3.1 方法总体框架（3.1.1/3.1.2）、3.2 流程约束架构（3.2.1-3.2.3）、3.3 能力-代码双层知识模型（3.3.1-3.3.4，含表3-4 相关工作对比）、3.4 Spec 增量知识演化机制（3.4.1-3.4.3）、3.5 图谱驱动的任务上下文生成（3.5.1-3.5.4）、3.6 本章小结；图3-1~3-5、表3-1~3-5、式 (3-1)-(3-4)、算法 3-1。
...
| `import_chapters.py` | md 第 1-5 章 -> docx 对应章区重建（含样式克隆、列表/表格/代码/粗体转换） |
| `convert_math.py` | docx 内 LaTeX 公式（`$$..$$` 与行内 `$..$`）-> Word 原生 OMML 公式 + 右侧编号 |
| `unify_refs.py` | 删章末引用块 -> 文末按 GB/T 7714-2015 建 59 条文献表 -> 正文引用转上标 |
...
# 1. 重建章节（会重置第1-5章区域，公式回到LaTeX文本，引用回到普通文本）
...
四步**顺序不可换**。如果改了参考文献清单（unify_refs.py 的 REFS 列表，现为 [1]-[59]），先更新 `unify_refs.py` 顶部的 `REFS` 列表与 `ok_citation` 上限再跑第 3 步。章节图片脚本按 Markdown 图像占位符定位，不按段落索引定位；当前覆盖图3-1至图3-5、图4-1至图4-5，后续章节应按同一模式扩展映射。
...
13. **`d.save` 报 PermissionError [Errno 13]** -> docx 正被 Word 打开（目录可见 `~$xxx.docx` 锁文件）-> 请用户关闭文档后重跑；脚本在 save 前崩溃不会损坏磁盘文件（保命设计）。另注意 `unify_refs.py` 的 `ok_citation` 引用号上限要与文献总数同步（现为 42）。
...
1. 更新目录域：Ctrl+A -> F9；
2. 补插图：图3-1~3-5、图4-1~4-5 由 `scripts/embed-ch3-figures-docx.py` 按 Markdown 图像占位符自动嵌入；其他章节图（如图5-1~5-4）仍需按各章 md 末尾「本章图表清单」补插。图片回填后，图下只保留规范图题，不能保留 Visio 源文件说明或其他占位性描述；
...
## 八、当前状态快照（2026-08-23，第 3 章六节重构成稿导入 docx+LaTeX 后）
...
| 第 3 章 | ✅ **六节重构成稿已导入并完成图表同步**（3.1 方法总体框架（闭环总图+三机制与三目标映射）/ 3.2 流程约束架构（六阶段+三段式契约+状态留痕+上下文请求与消费）/ 3.3 能力-代码双层知识模型（本体/多语言解析/多源证据/表3-4 RIG/KGCompass 对比）/ 3.4 Spec 增量知识演化机制（触发单元/冷启动/沉淀与增量更新）/ 3.5 图谱驱动的任务上下文生成（算法 3-1+式 3-1~3-4）/ 3.6 小结；图3-1~3-5 已生成 PNG/Visio、嵌入 Word 并同步 LaTeX；旧占位"待导入"已清零；小节号引用违规已清除） |
| 第 5 章 | ✅ 结构未变（表5-1 仍用 RQ1-RQ3 表述，与第 1 章新口径的统一待处理；"第 3 章表3-4"引用已改"表3-5"） |
| 第 4 章 | ✅ 成稿已导入并完成图表同步（4.1 系统架构、4.2 工作流模块、4.3 图谱构建、4.4 上下文生成、4.5 增量维护、4.6 系统测试、4.7 小结；图4-1~图4-5 已嵌入 Word 并同步 LaTeX） |
| 第 6 章 | ⬜ 占位待写（素材在 index.填充版备份.md） |
| 公式 | ✅ OMML 原生（显示 9 处 + 行内 10 段，失败 0） |
| 参考文献 | ✅ 59 条 GB/T 7714 + 正文上标（[1]-[42] 已终版核实；[43]-[57][59] 第 2 章新增、题录未逐条核实；[58] ContextBench 为**占位条目待题录**；作者前三位补全待 Word 端） |
| LaTeX 版 | ✅ 已同步（chap1-5 成稿，chap6 骨架；myrefs.bib 尚未建，引用为明文 [n]） |
